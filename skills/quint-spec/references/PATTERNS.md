...
def feeOf(pool: PoolId): int =
  if (cumulativeFees.contains(pool)) cumulativeFees.get(pool) else 0

def lastClaimed(user: Address, pool: PoolId): int =
  if (lastClaimedFees.contains((user, pool))) lastClaimedFees.get((user, pool)) else 0
...
val shareAccountingSound =
  USERS.forall(user =>
    val userShares = if (shares.contains(user)) shares.get(user) else 0
...
pure def nextSeqOrOne(seqs: (ChainId, ChannelId) -> int, chain: ChainId, channel: ChannelId): int =
  if (seqs.contains((chain, channel))) seqs.get((chain, channel)) else 1

def packetQueue(chain: ChainId, channel: ChannelId): List[Packet] =
  if (packetQueues.contains((chain, channel))) packetQueues.get((chain, channel)) else []
...
module BankKeeper {
  ...
  def addBalance(
    ...
    val account = if (bals.contains(addr)) bals.get(addr) else Map()
    val current = if (account.contains(denom)) account.get(denom) else 0
    bals.set(addr, account.set(denom, current + delta))
  }
...
module StakingKeeper {
  ...
  def delegatedAmount(delegator: Address, validator: Address): int =
    if (delegations.contains((delegator, validator)))
  ...
  action delegate(delegator: Address, validator: Address, amount: int): bool = all {
    ...
    delegations' = delegations.set((delegator, validator), delegatedAmount(delegator, validator) + amount),
  }
}
```
...
def allocatedOf(user: str): int = if (allocated.contains(user)) allocated.get(user) else 0
...
action allocate(user: str, amount: int): bool = all {
  ...
  allocated' = allocated.set(user, allocatedOf(user) + amount),
}
